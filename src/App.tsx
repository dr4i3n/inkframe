...
const TRANSLATIONS: Record<Language, any> = {
  EN: {
    ...
    hintDither: "Usually leave this OFF — CrossPoint/CrossInk firmware dithers full-grayscale images itself, and better. Turn it ON for stock firmware (1-bit) or a deliberate halftone / retro look. F-S = detailed, Atkinson = punchy, Bayer = even grid.",
  ...
  CS: {
    ...
    hintDither: "Většinou nech VYP — firmware CrossPoint/CrossInk si plný grayscale rozditheruje sám a líp. Zapni ho na stock firmwaru (1-bit) nebo na záměrný halftone / retro efekt. F-S = detailní, Atkinson = úderný, Bayer = pravidelný rastr.",
  ...
  DE: {
    ...
    hintDither: "Normalerweise AUS lassen — CrossPoint/CrossInk-Firmware dithert Vollgraustufenbilder selbst, und besser. Für Stock-Firmware (1-bit) oder einen bewussten Halbton-/Retro-Look einschalten. F-S = detailliert, Atkinson = kräftig, Bayer = gleichmäßiges Raster.",
  ...
  ES: {
    ...
    hintDither: "Normalmente déjalo en NO — el firmware CrossPoint/CrossInk tramifica la escala de grises él mismo, y mejor. Actívalo en firmware de fábrica (1 bit) o para un efecto de semitono / retro deliberado. F-S = detallado, Atkinson = con fuerza, Bayer = rejilla uniforme.",
  ...
  FR: {
    ...
    hintDither: "Laissez-le en général sur NON — le firmware CrossPoint/CrossInk trame lui-même les niveaux de gris, et mieux. Activez-le pour le firmware d'origine (1 bit) ou un effet demi-teinte / rétro voulu. F-S = détaillé, Atkinson = punchy, Bayer = grille régulière.",
  ...
  ZH: {
    ...
    hintDither: "通常保持「关」——CrossPoint/CrossInk 固件会自己抖动全灰度图，而且更好。原厂固件（1 位）或需要刻意半调/复古效果时才开。F-S=细腻，Atkinson=有力，Bayer=均匀网格。",
